...
````markdown
# DBM CME–ICME Association Tool (GUI)

A small PyQt5 GUI to associate an in-situ ICME to its likely solar CME source by **back-propagating** the event with the **Drag-Based Model (DBM)** and **propagating uncertainties** to obtain:
- a **departure time window at 20 R☉**
- an **initial speed window at 20 R☉**
- an optional **LASCO catalogue candidate list** consistent with those windows

This implementation follows the method described in the accompanying paper.

---

## What the GUI does

1. **Loads in-situ time series** (mission-dependent):
   - Magnetic field components and |B|
   - Proton density `n`
   - Flow speed `v` (radial/bulk)
   - Proton temperature `T` (when available)
...
2. **Lets you define event boundaries** (manual, in UTC):
   - Sheath start
   - Magnetic Obstacle (MO) *narrow* interval (start/end)
   - Magnetic Obstacle (MO) *wide* interval (start/end)
...
Save the script as something like `cme_icme_gui.py`, then:

```bash
python cme_icme_gui.py
```
